...
class FacialDetector:
    ...
    def get_positive_descriptors(self):
        ...
        self.params.number_positive_examples = num_images
        if self.params.use_flip_images:
            self.params.number_positive_examples *= 2
        ...
        for i in range(num_images):
            ...
            features = hog(img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                           cells_per_block=(self.params.cells_p_block, self.params.cells_p_block), feature_vector=True)
            print(len(features))
    ...
    def get_negative_descriptors(self):
        ...
        files = glob.glob(images_path)
        num_images = len(files)
        self.params.number_negative_examples = num_images
    ...
    def get_hard_mining_descriptors(self):
            # in aceasta functie calculam descriptorii negativi
            # vom returna un numpy array de dimensiuni NXD
            # unde N - numar exemplelor negative
            # iar D - dimensiunea descriptorului
            ...
            images_path = os.path.join(self.params.dir_hard_mining, '*.jpg')
            files = glob.glob(images_path)
            num_images = len(files)
            self.params.number_hard_mining = num_images

            hard_mining_descriptors = []
            print('Calculam descriptorii pt %d imagini hard mining' % num_images)
            for i in range(num_images):
                print('Procesam exemplul hard mining numarul %d...' % i)
                img = cv.imread(files[i], cv.IMREAD_GRAYSCALE)
                # TODO: sterge
                features = hog(img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                            cells_per_block=(self.params.cells_p_block, self.params.cells_p_block), feature_vector=True)
                ...
                hard_mining_descriptors.append(features)
            hard_mining_descriptors = np.array(hard_mining_descriptors)
            return hard_mining_descriptors

    def train_classifier(self,svm_file_name, training_examples, train_labels):
    ...
    def intersection_over_union(self, bbox_a, bbox_b):
        ...
        if box_a_area <= 0 or box_b_area <= 0:
            return 0 
            print("\n\n\nnumarator 0\n\n\n")
            print(f"box_a_area: {box_a_area} box_b_area: {box_b_area} inter_area: {inter_area}")
            print(f"a0: {bbox_a[0]} a1: {bbox_a[1]} a2: {bbox_a[2]} a3: {bbox_a[3]} ")
            print(f"b0: {bbox_b[0]} b1: {bbox_b[1]} b2: {bbox_b[2]} b3: {bbox_b[3]} ")
            
        size_factor = box_b_area/box_a_area

        iou = inter_area / float(box_a_area + box_b_area - inter_area)

        return iou

    def non_maximal_suppression(self, image_detections, image_scores, image_size, img):
        """
        ...
        if self.params.task == 2:
            sorted_image_detections = sorted_image_detections[is_maximal]
            sorted_scores = sorted_scores[is_maximal]
            
            max_score = sorted_scores[0]  
            score_threshold = max_score / 2

            valid_indices = sorted_scores >= score_threshold
            filtered_detections = sorted_image_detections[valid_indices]
            filtered_scores = sorted_scores[valid_indices]

            return filtered_detections, filtered_scores

        return sorted_image_detections[is_maximal], sorted_scores[is_maximal]

    def run(self,test_ratios, nr_of_resizing):
        """
        ...
        resize_factors = np.linspace(0.1,1,nr_of_resizing)
        for i in range(len(test_files)):
            ...
            for resize_factor in resize_factors:
                for r in test_ratios:
                    resized_img = cv.resize(img, (int(img.shape[1] * resize_factor * r), int(img.shape[0] * resize_factor)))
                    hog_descriptors = hog(resized_img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                                        cells_per_block=(self.params.cells_p_block, self.params.cells_p_block), feature_vector=False)
                    ...
                    for y in range(0, num_rows - num_cell_in_template):
                        for x in range(0, num_cols - num_cell_in_template):
                            ...
                            if score > self.params.threshold:
                                x_min = int(x * self.params.dim_hog_cell / resize_factor/r)
                                y_min = int(y * self.params.dim_hog_cell / resize_factor)
                                x_max = int((x * self.params.dim_hog_cell + self.params.dim_window) / resize_factor/r)
                                y_max = int((y * self.params.dim_hog_cell + self.params.dim_window) / resize_factor)
                                # print(np.var(img[y_min:y_max, x_min:x_max]))
                                if np.var(img[y_min:y_max, x_min:x_max]) > 1500:
            ...
            if len(image_scores) > 0:
                image_detections, image_scores = self.non_maximal_suppression(np.array(image_detections),
                                                                                np.array(image_scores), img.shape, img)
            if len(image_scores) > 0:
